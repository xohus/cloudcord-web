...
function BridgePanel() {
    ...
    const accountApi: any = (Vencord as any).Webpack?.findByProps?.("getAccounts", "switchAccount") || (Vencord as any).Webpack?.findByProps?.("getAccountIds", "switchAccount");
    const accounts = accountApi?.getAccounts?.() ?? accountApi?.getAccountIds?.()?.map((id: string) => ({ id })) ?? [];
    return <div style={{ maxWidth: 620 }}><div style={{ display: "flex", gap: 8, marginBottom: 18 }}><Button onClick={() => setTab("profile")}>Fake Profile</Button><Button onClick={() => setTab("botcord")}>BotCord</Button><Button onClick={() => setTab("accounts")}>Accounts</Button></div>{tab === "profile" ? <div>{field("Display name", "globalName")}{field("Pronouns", "pronouns")}{field("Bio", "bio")}{field("Nitro subscriber date", "nitroSince", "date")}<Button onClick={async () => { setStatus("Saving…"); try { await publish(profile); setStatus("Profile synchronized"); } catch (e) { setStatus(String(e)); } }}>Save & Sync</Button><div style={{ marginTop: 8 }}>{status}</div></div> : tab === "botcord" ? <div><h3>BotCord</h3><p>Open the limited BotCord compatibility interface. Bridge does not store bot credentials.</p><Button onClick={() => window.open(`${API}/botcord`, "_blank")}>Open BotCord</Button></div> : <div><p>Uses accounts already saved by Discord. CloudCord never reads or stores account tokens.</p>{accounts.map((account: any) => <div key={account.id ?? account.userId} style={{ display: "flex", justifyContent: "space-between", padding: 12 }}><span>{account.username ?? account.globalName ?? account.id ?? account.userId}</span><Button onClick={() => accountApi.switchAccount(account.id ?? account.userId)}>Switch</Button></div>)}{!accounts.length && <p>Discord has no additional saved accounts.</p>}{accountApi?.openAccountSwitcher && <Button onClick={() => accountApi.openAccountSwitcher()}>Add or manage accounts</Button>}</div>}</div>;
}
