...
import { isSincordPluginDev, isPluginDev } from "@utils/misc";
...
function CheckBadge({ badge, author }: { badge: string; author: User; }): JSX.Element | null {
    ...
    switch (badge) {
        case "SincordDonor":
            return (
                <span style={{ order: settings.store.SincordDonorPosition }}>
                    {badges.getSincordDonorBadges(author.id)?.map(badge => (
                        <RoleIconComponent
                            key={author.id}
                            className={roleIcon}
        ...
        case "SincordContributer":
            return isSincordPluginDev(author.id) ? (
                <span style={{ order: settings.store.SincordContributorPosition }}>
                    <RoleIconComponent
                        className={roleIcon}
                        name="Sincord Contributor"
                        size={20}
...
function ChatBadges({ author }: { author: User; }) {
    ...
    return (
        <span className="vc-sbic-badge-row" style={{ margin: "2px" }}>
            {settings.store.showSincordDonor && <CheckBadge badge={"SincordDonor"} author={author} />}
            {settings.store.showSincordContributor && <CheckBadge badge={"SincordContributer"} author={author} />}
...
export default definePlugin({
    ...
    authors: [Devs.Inbestigator, SincordDevs.KrystalSkull],
