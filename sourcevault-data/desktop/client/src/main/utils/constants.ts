...
export const DATA_DIR = process.env.CLOUDCORD_USER_DATA_DIR ?? (
    process.env.DISCORD_USER_DATA_DIR
        ? join(process.env.DISCORD_USER_DATA_DIR, "..", "CloudCordData", suffix)
        : join(app.getPath("userData"), "..", "CloudCord", suffix)
);
    ...
    if (!migrated) {
        setTimeout(() => {
            try {
                ...
                console.error("[CloudCord] Failed to copy prod data:", err);
            }
        }, 5000);
    }
}
